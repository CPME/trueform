...
const tests = [
  {
    name: "occt split placeholder: split face executes against solid owner",
    fn: async () => {
      const { backend, occt } = await getBackendContext();
      const part = dsl.part("split-placeholder", [
  ...
  {
    name: "occt split placeholder: split face fails when selector does not resolve to faces",
    fn: async () => {
      const { backend } = await getBackendContext();
      const part = dsl.part("split-placeholder-invalid", [
        dsl.sketch2d("sketch-base", [
      ...
      assert.throws(
        ...
        (err) =>
          ...
          err.message.includes("split face selector must resolve to faces")
      );
    },
  },
];
