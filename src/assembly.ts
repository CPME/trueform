...
function residualsFor(
  ...
  for (const mate of mates) {
    ...
    if (mate.kind === "mate.fixed") {
      residuals.push(...vecSub(bFrame.origin, aFrame.origin));
    ...
    if (mate.kind === "mate.planar") {
      ...
      residuals.push(dot(vecSub(bFrame.origin, aFrame.origin), aFrame.zAxis) - offset);
      continue;
    }
    if (mate.kind === "mate.coaxial") {
      residuals.push(...cross(aFrame.zAxis, bFrame.zAxis));
      residuals.push(...cross(vecSub(bFrame.origin, aFrame.origin), aFrame.zAxis));
      continue;
    }
  }
  return residuals;
